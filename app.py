...
def get_words():
    ...
    random_words = wordnik_service.get_random_words(
        minLength=5, maxLength=10, limit=500
    )
...
def middle_frame(container):
    ...
    def countdown(seconds):
        ...
        if seconds > 0 and not stop_signal:
            ...
            text_box.insert(
                END,
                f"Your accuracy was {final_accuracy}% and you typed {WPM} words per minute.",
            )  # noqa: F405
            window.focus_set()
            bottom_frame.delete("1.0", "end")
            bottom_frame.insert("1.0", "Click restart above to try again.")
    ...
    bottom_frame = Text(
        frame, width=30, height=1, wrap=WORD, font=("Arial", 30), padx=20, pady=20
    )  # noqa: F405
            ...
            if len(current_word_list) > 0:
                ...
                if event.char == current_word_list[0]:
                    ...
                    if word_count == 0:
                        ...
                        found_char = text_content.find(
                            current_word_list[0], char_convert
                        )
                        ...
                        if found_char != -1:
                            ...
                            text_box.tag_add(
                                "font", found_char_position, next_char_position
                            )
                        ...
                        if old_start_index == 0:
                            ...
                            start_index_convert_dec = (
                                int(old_start_index) + original_start_index
                            )
                            old_start_index += 1

                        start_index_convert_dec_string = str(start_index_convert_dec)
                        start_index = (
                            start_index_convert[0]
                            + "."
                            + start_index_convert_dec_string
                        )
                        text_content = text_box.get(start_index, end_index)
                        char_convert = char_index.split(".")[1]
                        char_convert = int(char_convert)
                        ...
                        if found_char != -1:
                            found_char_position = (
                                char_whole + "." + start_index_convert_dec_string
                            )
                            next_char_position = found_char_position + "+1c"
                    ...
                    if word_count == 0:
                        ...
                        text_content = text_box.get(start_index, end_index)
                        found_char = text_content.find(
                            text_content[char_index], char_index
                        )
                        if found_char != -1:
                            next_char_position = full_char_index + "+1c"
                            text_box.tag_add(
                                "wrong_font", full_char_index, next_char_position
                            )
                        ...
                        if found_char != -1:
                            ...
                            next_char_position = found_char_position + "+1c"
                            text_box.tag_add(
                                "wrong_font", start_index, next_char_position
                            )
                            text_box.tag_config("wrong_font", foreground="red")
                            current_letters.append(current_word_list.pop(0))
                        else:
